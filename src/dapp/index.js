...
(async () => {
	...
	let contract = new Contract('localhost', () => {
		...
		DOM.elid('buy-insurance').addEventListener('click', () => {
			...
			const amount = web3.utils.toWei(parseFloat(value), 'ether');
			console.log(amount);
				...
				display('Oracles', 'Trigger Oracles', [
					{
						...
						error: error,
						value: result.airline + ' ' + result.flight + ' ' + result.value,
					},
				]);
			});
		});
		...
		DOM.elid('add-funds').addEventListener('click', () => {
			...
			contract.addFunds((error, result) => {
				display('Oracles', 'Trigger Oracles', [
					{ label: 'Add Funds', error: error, value: JSON.stringify(result) },
				]);
			});
		});
	});
...
function display(title, description, results) {
	...
	results.map((result) => {
		...
		row.appendChild(
			DOM.div(
				{ className: 'col-sm-8 field-value break-word' },
